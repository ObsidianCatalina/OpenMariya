...
async def setu(app: Ariadne, group: Group, event: GroupMessage):
   if langmanager.en_us==1:
    ...
    await app.send_group_message(group, MessageChain(At(event.sender.id)," The language(en_us) is not available.If you installed it,you can type 'Lang --set-default-lang = en_us'"))
